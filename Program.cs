...
/// 
/// 原理根据日期分类
/// 选项 2: 仅归档流程 (Scan -> Archive / 移动)    —— Scan——》 Open
/// 
/// 原理：根据关键词分类，关键词需要清洗
/// 选项 3: 仅仅分类流程 (Scan -> Categorize / 移动)  
/// 
/// 原理：文件名称后缀带有@@@评分80【两位数字】提取到外层，输入和输出不允许改变，必须写死，永不改变
/// 选项 4: 指定评分图片移动到外层 (Score Organizer / 移动)
/// 
/// 原理：根据清洗后的tag得出对应的10个特征词，添加到文件夹后缀名里面
/// 选项 5: 仅自动添加 10 个 tag (Tagging Flow / 重命名)   扫描——数据清洗
/// 
/// 原理：根据特征词的评分去监控训练他们的特征进行只基于关键词的评分【我自己的“精选”文件夹里面会有一个大致的评分给他们做一个预估值】，如果有评分后缀，则刷新评分，没有评分则添加后缀
/// 选项 6: 仅自动添加评分 (Scoring and Tagging Flow / 重命名)
/// 
/// 
/// 选项 7: 完整流程 (Full Flow)
/// 扫描——出表
///     ——归档——分类——添加特征词——评分——手动选择评分【感觉可以自动提取95-99分】
///         ——再进行扫描——出表
/// 
/// 原理：提取"1girl"之前出现的一大串词，如果重复超过30次就提取出来，后期可以自行改超过多少次这样
/// 选项8：提取风格词，目的是为了用这个风格词清除后期添加的词语。
///         得先实现 功能1 和 功能8 再进行，功能3 可能有问题，移动文件有风险，必须要锁住特定的文件夹，避免被移动
/// 
/// 利用选项8得出来的风格词，进行清除文件名里面的风格词，避免风格词影响后期的关键词提取
/// 选项9：单独的提取核心词功能
/// 
/// 
/// 
